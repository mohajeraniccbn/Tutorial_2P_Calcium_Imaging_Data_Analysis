...
make_db_Suite2P; % RUN YOUR OWN MAKE_DB SCRIPT TO RUN HERE
...
ops0.RootStorage            = main_data_folder; % Suite2P assumes a folder structure, check out README file
ops0.temp_tiff              = fullfile(main_data_folder,'temp.tif'); % copies each remote tiff locally first, into this file
ops0.RegFileRoot            = processed_data_folder;  % location for binary file
ops0.DeleteBin              = 1; % set to 1 for batch processing on a limited hard drive
ops0.ResultsSavePath        = processed_data_folder; % a folder structure is created inside
ops0.RegFileTiffLocation    = processed_data_folder; %'D:/DATA/'; % leave empty to NOT save registered tiffs (slow)
...
ops0.PhaseCorrelation       = 0; % set to 0 for non-whitened cross-correlation
ops0.SubPixel               = 2; % 2 is alignment by 0.5 pixel, Inf is the exact number from phase correlation
...
ops0.NimgFirstRegistration  = 150; % number of images to include in the first registration pass 
...
for iexp = 1 %[3:length(db) 1:2]
    ...
    % this won't run on mac because deconvL0 has not been built with mex
    % function ... you might have to build your own deconvL0.mex file from
